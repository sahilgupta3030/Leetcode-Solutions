...
public:
    ...
    std::string shortestCommonSupersequence(std::string str1,
        ...
        for (int i = 1; i <= m; ++i) {
            for (int j = 1; j <= n; ++j) {
                if (str1[i - 1] == str2[j - 1]) {
                    // Characters match, increment the length of common
                    // subsequence
                    dp[i][j] = dp[i - 1][j - 1] + 1;
                } else {
                    // Characters do not match, take the maximum from either the
                    // top or left cell
        ...
        while (i > 0 || j > 0) {
            if (i == 0) {
                // If we have reached the beginning of str1, append remaining
                // str2
                sequence += str2[--j];
            } else if (j == 0) {
                // If we have reached the beginning of str2, append remaining
                // str1
                sequence += str1[--i];
            } else {
                ...
                if (dp[i][j] == dp[i - 1][j]) {
                    ...
                    // When characters match, append one of them and move
                    // diagonally
                    sequence += str1[--i];
                    --j;
                }
